...
run_training() {
    ...
    local HYDRA_RUN_DIR="./outputs/downstream/nt/${TASK}/hnet_ntv2_data${dataset_name}_lr-${lr}_val_idx-${val_idx}_pooling-${pooling}"
    rm -rf "${HYDRA_RUN_DIR}"
    mkdir -p "${HYDRA_RUN_DIR}"
    local CFG_PATH="/workspace/caduceus_proj/outputs/pretrain/hg38/LARGE_hnet_ntv2_seqlen-4k_n_enc_layer-4_n_dec_layer-4_lr-3e-4_tokenizer_name-ntv2/model_config.json"
    local CKPT_PATH="/workspace/caduceus_proj/outputs/pretrain/hg38/LARGE_hnet_ntv2_seqlen-4k_n_enc_layer-4_n_dec_layer-4_lr-3e-4_tokenizer_name-ntv2/checkpoints/val/loss.ckpt"
    ...
    python -m train \
        ...
        dataset.tokenizer_name=ntv2 \
        dataset.add_eos=false \
        +dataset.conjoin_test=false \
        model._name_=dna_embedding_hnet_ntv2 \
        ...
        wandb.name=HNET_NTV2_${pooling}_${dataset_name}_val_idx-${val_idx}_lr-${lr} \
