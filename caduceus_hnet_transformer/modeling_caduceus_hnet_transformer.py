...
from hnet.rope import RotaryEmbedding as RopeRotaryEmbedding
...
class CrossAttentionUpsampler(nn.Module):
    ...
    def forward(self, query, key_value, key_padding_mask=None):
        ...
        attn_output = F.scaled_dot_product_attention(q, k, v, attn_mask=attn_mask, dropout_p=0.0)
...
@dataclass
class CaduceusHNetTransformerModelOutput(BaseModelOutputWithNoAttention):
...
class CaduceusEmbeddings(nn.Module):
    def __init__(
            self,
            config: CaduceusHNetTransformerConfig,
            device=None,
            dtype=None,
    ):
        super().__init__()
...
class RotarySelfAttention(nn.Module):
    def __init__(self, config: CaduceusHNetTransformerConfig, rotary_emb: RopeRotaryEmbedding):
        super().__init__()
    ...
    def forward(self, x, attention_mask=None):
        ...
        q = q.view(B, L, self.n_head, self.d_head)
        k = k.view(B, L, self.n_head, self.d_head)
        v = v.view(B, L, self.n_head, self.d_head)

        qkv = torch.stack([q, k, v], dim=2)
        
        self.rotary_emb(qkv)

        q_rot = qkv[:, :, 0].transpose(1, 2)
        k_rot = qkv[:, :, 1].transpose(1, 2)
        v_new = qkv[:, :, 2].transpose(1, 2)

        # Use scaled_dot_product_attention
        attn_output = F.scaled_dot_product_attention(q_rot, k_rot, v_new, attn_mask=attention_mask)
...
class CaduceusMixerModel(nn.Module):
    def __init__(
        ...
        rotary_emb = RopeRotaryEmbedding(dim=d_head, interleaved=False)
    ...
    def forward(self, input_ids, inputs_embeds=None, output_hidden_states=False, boundaries=None):
        ...
        if boundaries is not None:
            mask = boundaries[:, 1:] != 0
            if mask.any():
                boundary_loss = F.mse_loss(p_original[:, 1:][mask], boundaries[:, 1:][mask])
        ...
        attention_mask = None # Can be added if needed based on chunk_lengths
...
class CaduceusPreTrainedModel(PreTrainedModel):
    ...
    config_class = CaduceusHNetTransformerConfig
...
class CaduceusHNetTransformer(CaduceusPreTrainedModel):
    """Caduceus model that can be instantiated using HF patterns."""
    def __init__(self, config: CaduceusHNetTransformerConfig, device=None, dtype=None, **kwargs):
    ...
    def forward(
    ...
    ) -> Union[torch.Tensor, Tuple, CaduceusHNetTransformerModelOutput]:
        ...
        if return_dict:
            return CaduceusHNetTransformerModelOutput(
...
class CaduceusHNetTransformerForMaskedLM(CaduceusPreTrainedModel):
    """HF-compatible Caduceus model for masked language modeling."""

    def __init__(self, config: CaduceusHNetTransformerConfig, device=None, dtype=None, **kwargs):
        super().__init__(config, **kwargs)
        factory_kwargs = {"device": device, "dtype": dtype}
        self.caduceus = CaduceusHNetTransformer(config, **factory_kwargs, **kwargs)
...
class CaduceusHNetTransformerForSequenceClassification(CaduceusPreTrainedModel):
    def __init__(
            self,
            config: CaduceusHNetTransformerConfig,
