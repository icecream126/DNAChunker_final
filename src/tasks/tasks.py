...
class LMTask(BaseTask):
    def forward(self, batch, encoder, model, decoder, _state):
        ...
        model_kwargs = w if w is not None else {}
        ...
        if "state" in inspect.signature(model.forward).parameters.keys():
            ...
            output = model(x, **model_kwargs)
            state = None

        aux_losses = {}
        if isinstance(output, dict):
            aux_losses = {k: v for k, v in output.items() if 'loss' in k}
            x = output['logits']
        else:
            x = output

        self._state = state
        x, w_new = decoder(x, state=state, **z)
        if w is None: w = {}
        if w_new is None: w_new = {}
        w.update(w_new)
        w['aux_losses'] = aux_losses
